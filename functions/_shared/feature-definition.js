...
- \`사이트 오류/이슈 기록\`은 운영자가 직접 입력/수정/삭제하지 않고, 공개 홈의 자동 오류 보고와 사이트/관리자 전역 API 오류 로그를 함께 자동 집계하는 읽기 전용 패널로 사용한다.
...
**분석**
- \`GET /api/admin/analytics\` — 관리자 통계 대시보드. 방문/조회/유입 경로와 함께 최근 기간 기준 \`요일 × 시간 방문 히트맵\`, \`글머리 태그 워드 클라우드\`, \`태그 관계도\`를 포함한다. 방문 히트맵은 월요일부터 일요일까지 시간대별 방문 집중도를 진한색/옅은색으로 보여준다. 태그 인사이트 기간은 방문 분석 기간과 별도로 조절할 수 있고, 관계도는 \`글머리 태그 + 메타 태그\`를 합친 키워드 중심 그래프로 동작한다. 연결선은 기본적으로 보이고 hover 시 관련 관계가 더 선명해지며, 키워드를 클릭하면 선택 상태가 유지된 채 관련 기사 모달이 열린다. 다른 영역을 클릭하면 선택이 해제된다. 워드 클라우드 크기는 기사 수와 관련 기사 조회수를 함께 반영하며, 그래프는 drag/zoom 상호작용을 지원한다.
