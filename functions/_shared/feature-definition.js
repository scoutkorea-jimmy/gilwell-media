...
**분석**
- \`GET /api/admin/analytics\` — 관리자 통계 대시보드. 방문/조회/유입 경로와 함께 최근 기간 기준 \`글머리 태그 워드 클라우드\`와 \`태그 관계도\`를 포함한다. 태그 인사이트 기간은 방문 분석 기간과 별도로 조절할 수 있고, 관계도는 \`글머리 태그 + 메타 태그\`를 합친 키워드 중심 그래프로 동작한다. 연결선은 기본적으로 보이고 hover 시 관련 관계가 더 선명해지며, 키워드를 클릭하면 우측 정보 패널에 연결 키워드·기사 수·조회수·관련 기사 미리보기가 함께 갱신된다. 기사 전체 목록은 선택 키워드 기준 모달로 추가 확인한다. 워드 클라우드 크기는 기사 수와 관련 기사 조회수를 함께 반영하며, 그래프는 drag/zoom 상호작용을 지원한다.
