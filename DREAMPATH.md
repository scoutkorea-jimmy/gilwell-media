...
| Auth | `dp_session=1` httpOnly 쿠키 1h TTL + localStorage profile |
...
| `dp_tasks` | `id, title, assignee, status, priority, due_date, updated_at` |
...
아래 지점은 이미 사고 / 회귀가 있었습니다. 각 줄 위에 Section 17.3 포맷으로 주석이 붙어 있어야 합니다 (2026-04-24 초기 적용):

| 위치 | 일자 | 사건 요약 |
|---|---|---|
| `functions/_middleware.js` `isLegacyInlinePath()` | 2026-04-24 | `/dreampath` 누락 → 인라인 onclick 전체 차단 |
| `js/dreampath.js` IIFE 선언부 (`const DP = ...`) | 설계 원본 | IIFE 분리 시 인라인 onclick 참조 증발 |
| `js/dreampath.js` Tiptap 초기화 4곳 | 설계 원본 | 한 곳만 고치면 해당 경로에서만 조용히 기능 죽음 |
