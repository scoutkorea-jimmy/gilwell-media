...
| `dist-homepage/templates-app.js` `syncAutoPages` / `restoreFlowToFirst` / `paginateBox` / `makeSplitTail` | 2026-06-09 | 매 입력마다 문단 재배치 → 캐럿 파괴(fast-path + 캐럿 보존). 순서보존 분할: 넘치는 첫 노드+이후 전부를 다음 장으로, tail은 source-id 재결합. 인접 머지/끝노드 이동 금지(순서깨짐·폭주) |
| `js/dreampath.js` 템플릿 input 핸들러 / `_renderDocumentTemplates` | 2026-06-09 | 키 입력당 이중 flow-sync 제거(구조편집 경로는 유지) · iframe HTML 문서 `?v=` 캐시버스트 |
...
13. **pdf.js 워커는 same-origin self-host 고정** (`/js/vendor/pdf.worker.min.js`) — CSP `worker-src`가 `default-src 'self'`로 폴백하므로 CDN 워커는 차단된다. pdf.js 버전 올릴 때 워커 파일도 동일 버전으로 교체 (Section 22).

---

## Section 22 — 📚 Knowledge Base (Document Wiki)

**개발 배경 / Development background**
PMO가 다루는 운영 문서가 PDF·DOCX로 흩어져 있고, 같은 문서가 개정될 때 무엇이
어떤 맥락으로 바뀌었는지 추적·공유할 방법이 없었다. 2026-06-04 오너 요청으로
문서를 Dreampath 안에서 위키화하고 버전 변경을 추적·팔로업하는 기능을 신설.

**개발 목적 / Development purpose**
문서를 한곳에서 읽고(위키화), 같은 제목의 새 버전이 올라오면 단어 단위 diff +
변경 맥락 메모로 변경 이력을 남기며, PMO 구성원이 각 버전을 팔로업하고 개인
메모·댓글로 협업한다. 성공 기준: PDF/DOCX 업로드 → 추출 → 정리 → 저장 → 같은
제목 재업로드 시 diff·맥락이 보이고, 다른 사용자가 팔로업/댓글 가능.

**특이사항 / Remarks**
- 진입: 사이드바 **Overview → Knowledge Base** (`id:'wiki'`, `perm:view:wiki`).
- 추출은 **브라우저**에서: DOCX→`mammoth.js`(cdnjs), PDF→`pdf.js`(jsdelivr `3.11.174`). 서버 파싱 아님(Workers 제약).
- **pdf.js 워커는 same-origin 필수** — `/js/vendor/pdf.worker.min.js`. CDN 워커는 CSP `worker-src`(→`default-src 'self'`)에 차단. cMap은 jsdelivr(connect-src 허용)에서 로드해 한글 PDF 추출 보장. → `functions/_middleware.js` 미변경, Dreampath `./deploy.sh` 단일 경로.
- 정체성 = `slug`(정규화 제목). 같은 slug 재업로드 = 새 버전(`version_no = current+1`). `_slug()` 규칙 변경 시 기존 문서 분리 위험 (DREAMPATH-HISTORY.md 2026-06-04 케이스 2).
- Tiptap **재사용만**(신규 extension 아님) → §4.3 4-spot 규칙 비해당. 신규 extension 추가 시에만 4곳 동시 갱신.
- 권한: `view:wiki`(읽기·팔로업·댓글) / `write:wiki`(생성·편집). PUT은 작성자 본인도 가능. DELETE은 admin.
- 파일: `functions/api/dreampath/wiki.js`, `wiki-comments.js`, `js/dreampath.js`(위키 모듈), `dreampath.html`(로더+CSS), `js/vendor/pdf.worker.min.js`. 테이블 `dp_wiki_pages/_versions/_followups/_comments` (`db/dp_wiki_2026-06-04.sql`).
- 상세 화면은 **본문/변경보기 토글**(`_wikiDetailMode`): 평소엔 최신 본문만, "변경 보기"를 켜면 상단 본문 자리(`#dp-wiki-main`)에 비교가 열림.
- diff는 **문단 단위 LCS(`_docDiffRows`) 후 바뀐 문단 안에서만 단어 하이라이트** — 기본 **좌우 2단(현행|개정, `_renderDiffSplit`)**, "인라인" 토글(`_renderDiffInline`). 동일 문단 접기 + "변경된 부분만" 토글. 전체를 단어로 비교하면 노이즈·성능 문제라 문단 정렬이 먼저.
- **변경마다 "변경 사유" 메모**: `dp_wiki_diff_notes`(from/to 버전쌍 + `row_key`=변경텍스트 해시). `view:wiki`면 추가/수정. diff 행 아래 인라인 표시(`_diffNoteHtml`). 버전 in-place 편집 시 해시 바뀌면 메모 정렬 어긋날 수 있음(알려진 트레이드오프).
- **페이지 식별 = 기본 제목**(`_baseTokens`/`_slug`): 끝의 버전/초안 표식(`v1.2`·`1.0`·`DRAFT`·`rev2`)만 떼어 같은 문서로 묶음. 단순 연도/정수는 유지(오병합 방지). 자동 판단이 빗나가면 업로드 시 `attach_page_id`로 수동 연결. (DREAMPATH-HISTORY.md 2026-06-04 케이스 3)
- **신구대조표(old/new) 자동 비교**: DOCX 표를 파싱해 현행/개정 열 자동 인식 → 행별 `_wordDiff`로 변경점 생성, 개정안을 위키 버전으로 저장 가능. **DOCX 전용**(PDF는 표 구조 소실). 코드 `_wikiCompareDetect/_wikiCompareRender`.
...
2. **Scope is Dreampath only.** Version notes may mention only:
   ...
   `dp_*` db/ migrations·tables, or `deploy.sh` when it affects Dreampath deploys.
...
4. **Storage format**: `dp_versions.description` uses JSON:
   ```json
   {
     ...
     "background": { "en": "...", "ko": "..." },
     "changes": [{ "en": "...", "ko": "..." }],
     "implementation": [{ "en": "...", "ko": "..." }],
     "context": { "en": "...", "ko": "..." }
   }
   ```
   `background` explains why the release happened for non-developers.
   `implementation` explains the technical work in plain language.
...
5. **Display format**: the Versions page renders each row as:
   - Version number
   - Feature summary with visually separated EN / KO cards
   - Background / context with EN / KO cards
   - `| English | Korean |` table for changed behavior
   - `| English | Korean |` table for implementation details
6. **`deploy.sh` may auto-append git commits**, but only from the current
   unpushed release range (`origin/main..HEAD`) after path filtering to
   ...
   and old Dreampath release subjects into new rows.
