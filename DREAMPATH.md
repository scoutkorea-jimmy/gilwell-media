...
- diff는 **문단 단위 LCS(`_docDiffRender`) 후 바뀐 문단 안에서만 단어 하이라이트(`_wordDiff`)** — 전체를 단어로 비교하면 노이즈·성능 문제. 동일 문단은 접어서("⋯ 동일 N문단 ⋯") 변경점 중심으로 보여주고 "변경된 부분만" 토글 제공. `<ins>`/`<del>` 시맨틱(색+밑줄/취소선, 색각이상 대응).
