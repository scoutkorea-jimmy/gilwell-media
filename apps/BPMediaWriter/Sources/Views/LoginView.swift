...
struct LoginView: View {
    ...
    @State private var username = "owner"
    ...
    private func submit() async {
        ...
        // First attempt: without Turnstile unless we already have a token / site key.
        do {
    ...
    private func attemptLogin(username: String, password: String, turnstile: String?) async throws {
        // Use AppState.login which maps errors to alerts; rethrow via a local call for Turnstile branching.
        do {
            let result = try await appState.api.login(username: username, password: password, turnstileToken: turnstile)
            appState.auth.saveSession(token: result.token ?? "", role: result.role, user: result.user)
