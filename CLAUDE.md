...
- `6` — 작업 효율성 노트 (Site/Admin 공통 함정·패턴)
...
## 6 작업 효율성 노트 (Site / Admin)

> [!info] 세션 인사이트 누적
> 실제 작업 중 반복적으로 발견한 함정·패턴을 모은 체크리스트. 새 기능 착수 전이나 회귀 디버깅 전에 한 번 훑으면 30분~1시간 절약됨. 새 인사이트는 발견 즉시 이 섹션에 prepend로 추가.

### 6.1 Enum / 카탈로그 변경은 "원본 1곳 → 동기화 N곳"
...
### 6.2 `functions/api/settings/*` PUT 권한 게이트 정렬
...
### 6.4 admin은 `css/style.css`를 로드하지 않음
...
### 6.8 `homepage_issues` 자동 보고 severity 표준
