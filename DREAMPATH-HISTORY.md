...
**후속 (01.068.00)**: (a) 떠 있는 캔버스 편집 도구(↕/+ 블록툴, B/H 포맷바, 삽입메뉴)를
마지막 상호작용 후 **5초 뒤 디졸브**(opacity transition)로 숨김 — 도구에 호버하면 일시정지
(`js/dreampath.js` `_templateArmToolFade/_templateClearToolFade` + 주입 CSS `.dp-tool-dissolve`).
(b) 문서번호 규칙을 `DP-DOC-YYYY-AAA1234` → **`DP-DOC-YYYYMMDD-{작성자 이름 첫자}{랜덤 영문 2}{랜덤 숫자 4}`**
로 변경(`templates-app.js` `docNo`, 부모가 `__DP_AUTHOR_INITIAL` 주입 후 1회 재생성;
`js/dreampath.js` `_templateDocNumber`/추출 regex). (c) **Notion식 마크다운**: 본문/섹션
블록에서 `# `/`## `/`### `→제목(표준: #가 가장 큼), `- `/`* `→불릿, `1. `→번호, **Tab/Shift+Tab
...
**케이스 3 — 파일명 버전 접미사가 한 문서를 여러 페이지로 쪼갬 (v01.059.01 수정)**
- 증상: `CUFS_..._v1.2_DRAFT` 와 `CUFS_..._v1.0_DRAFT` 가 서로 다른 위키 페이지로 생성됨(같은 문서여야 함).
- 원인: slug가 `v1.2`/`DRAFT` 접미사를 그대로 포함 → 다른 slug → 다른 페이지.
- 수정: `_baseTokens()`로 **끝에 붙은 버전/초안 표식만** 제거(`v\d`, 점버전 `1.2`, `draft/final/rev`) 후 slug 생성. 페이지 식별 = 기본 제목. 단순 연도(`Budget 2026`)·정수는 표식으로 보지 않아 오병합 방지. `page.title`=기본 제목, `version.title`=업로드 원본 파일명(타임라인 표시).
- 보완: 업로드 모달에 "기존 문서에 연결" 수동 선택(`attach_page_id`) + 실시간 안내. 자동 판단이 빗나가도 직접 연결 가능.
- 기존 분리된 행은 1회성 머지(SQL)로 한 페이지로 합침.

**신구대조표(old/new comparison table) 자동 비교 (v01.060.00)**
- 기능: DOCX 신구대조표를 올리면 표를 파싱해 현행/개정 열을 자동 인식(헤더 키워드: 현행·원안 / 개정안·개정), 행별로 `_wordDiff`를 돌려 "변경 내용"을 자동 생성. 4열(구분·현행·개정안·개정 사유) 레이아웃이면 구분·사유 열도 함께 표시. "개정안을 위키 버전으로 저장" 지원.
- 제약: **DOCX 전용** — PDF는 텍스트 추출 시 표 구조가 사라져 열 분리가 불가능. 코드: `js/dreampath.js` `_wikiCompareDetect/_wikiCompareRender`.
