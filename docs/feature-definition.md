...
- 형식: `V3.aaa.bb`
  - `V3` : 제품 단계. 오너만 수정한다.
  - `aaa` : 기능 추가 (001부터 시작, 세 자리 zero-pad).
  - `bb`  : Hotfix / Bugfix 전용 (00부터 시작).
- 예시: `V3.001.00` → `V3.001.01` (버그픽스) → `V3.002.00` (새 기능)
- `VERSION` 파일에 `3.aaa.bb` 형태로 저장하며, HTML/JS에서는 `V3.aaa.bb` 로 표시한다.

#### 각주
- 기능 추가 없이 버그만 고치면 bb만 올린다. aaa가 올라가면 bb는 00으로 초기화한다.
