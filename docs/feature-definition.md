...
- 403 응답 UX (03.100.00): 멤버가 권한 없는 API를 호출하면 서버가 내려주는 한국어 메시지("이 메뉴의 보기 권한이 없습니다. 오너에게 요청하세요.")가 토스트로 6초간 표시된다. 403은 `/api/homepage-issues/report` 자동 신고 대상에서 제외 (권한 거부는 버그가 아님).
